...
Joycss.prototype = {
  ...
  _readFromDir: function(file){
    ...
    Api.getImagesSize(images, function(err, sizes){
      ...
      utils.forEach(sizes, function(size, filename){
        ...
        text = text + '.' + classname + ' { background: url(' + file + ');' +
          'width: ' + size.width + 'px; height: ' + size.height + 'px; display: inline-block; *zoom: 1; *display: inline; }';
      });
  ...
  _bind: function(){
    ...
    spriteDef.on('finish:parser', function(){
      ...
      if (!nochange){ 
        ...
        Joycss.Event.emit('run:end', { file: cssWrite.get('destFile') });
      }
    });
  ...
  _writeConfig: function(){
    ...
    fs.writeFile(file, text, function(err){
      ...
      if (err) {
        Log.error('write config false. ' + time, 'joycss end');
        Log.error(err);
      } else {
        Log.success('File ' + cssWrite.get('destFile') + ' create. ' + time);
      }
