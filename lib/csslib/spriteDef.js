...
StdClass.extend(SpriteDef, StdClass, {
  ...
  initSpriteConf: function(params){
    ...
    if (imgPath) {
      ...
      if (this.get('relative')) {
        this.sprites[spriteId]['relative'] = this.get('relative') + '/' +
                         spriteId + '-sprite.png';
      }
    }
  ...
  _setChangedRules: function(sprite){
    ...
    var extraRule = {
      property: ['_background-image', 'background-image', 'background-repeat'],
      value: ['url(' + imgPath + ')', 'url(' + imgPath.replace('sprite8','sprite') + ')', 'no-repeat',],
      selector: selectors
    };
