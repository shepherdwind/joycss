...
Box.prototype.setEM = function(){
  ...
  if (lineH > -1){
    ...
    if (property.attributes.units == 'px'){
      this.lineH = property.attributes.value;
    }
  }

  var em = this.property.indexOf('font-size');
  var font = this.property.indexOf('font');

  if (em > -1) {
    var property = new Property("font-size",this.value[em]);
    if (property.attributes.units == 'px'){
...
Box.prototype.getProp = function(prop, i){
  ...
  switch(prop) {
    ...
    case 'padding': {
      this.width += attr.padding[1] + attr.padding[3];
    ...
    case 'height': {
      if (attr.units === 'px') {
        this.hasHeight = true;
        this.height += attr.value > this.lineH? attr.value : this.lineH;
      }
      break;
    }
