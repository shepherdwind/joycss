...
StdClass.extend(CssReader, StdClass, {
  ...
  parse: function(str){
    ...
    if (str) this.set('text', str, false);
    var text;
    if (file) {
  ...
  _ruleEnd: function ruleEnd(e){
    ...
    if (!this.attributes.nest.length){
      var num = this.attributes.selectors.length - ruleEnd.recode;
      //console.log('num is %s, recode is ', num, ruleEnd.recode);
      ruleEnd.recode = 0;
  ...
  _ruleStart: function ruleStart(e){
    ...
    this.attributes.nest.push(this.attributes.selectors.length);
    this._addSelector(e);
  },
  ...
  _addValue: function addValue(e){
    ...
    if (e.old === 'valueStart') {
      ...
      data += e.data;
      idList.push({type: "meta", value: data});
      metas[len].push(data);
    }
  },
  ...
  _addComment: function(comment){
    var idList = this.get("idList");
    idList.push({type: "comment", value: comment});
  },

  iterator: function(fn, context){
    var idList = this.get('idList');
    var self = this;
    idList.forEach(function(index, i){
      var rule = index.type ? index : self.getRule(index);
  ...
  getRule: function(i){
    ...
    i = parseInt(i, 10);
    if(this.attributes.selectors.length > i){
      return {
  ...
  getLen: function(){
    return this.attributes.idList.length;
  },
  ...
  _read: function read(data){
    ...
    while(code){
      ...
      if (code === 10 || code === 13){
        ...
        //comment end 
        comment = false;
        this._addComment(data[slice](j, i + 2));
        i++;
        j = i + 1;
